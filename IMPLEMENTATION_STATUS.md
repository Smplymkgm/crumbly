...
### Login (25 de agosto de 2026) — Google Sign-In + correo/contraseña, a elección

> ⚠️ **Superado el 29 de agosto de 2026** — ver la sección siguiente. El token fijo `CRUMBLY_TOKEN` que este bloque todavía usaba por debajo (Google/correo lo intercambiaban por él) se eliminó por completo, reemplazado por sesiones propias de cada dispositivo. Esta sección queda como registro histórico de cómo se llegó ahí — no como especificación vigente.

El usuario insistió en ir más allá del link de un toque: quería que cualquier dispositivo nuevo solo necesitara "ingresar y todo esté listo, nada local" — sin copiar URL/token a mano ni depender de un link guardado. Primero se construyó un login con correo + contraseña corta (decisión explícita del usuario de no reutilizar el token largo como contraseña). El mismo día, el usuario pidió también iniciar sesión con Google — se agregó **Google Identity Services** como método principal (botón "Iniciar sesión con Google"), con el trade-off de configuración (requiere un proyecto en Google Cloud Console) explicado y confirmado por el usuario antes de construirlo, **manteniendo** el correo+contraseña como segundo método visible en la misma pantalla (no como fallback técnico escondido) — pedido explícito tras ver que el primer diseño solo mostraba el escape hatch de URL/token en vez de un login limpio.

| Pieza | Estado | Verificación |
|---|---|---|
| `loginGoogle_(body)` en `backend/Code.gs` | ✅ DONE | Acción `loginGoogle`, resuelta en `doPost` **antes** del chequeo de token. Recibe el ID token (JWT) que Google entrega en el navegador y lo **verifica contra Google** (`oauth2.googleapis.com/tokeninfo`, no confía en decodificarlo solo) — chequea `aud` contra `CRUMBLY_GOOGLE_CLIENT_ID`, `email_verified`, y que el correo sea exactamente `CRUMBLY_LOGIN_EMAIL`. |
| `login_(body)` en `backend/Code.gs` | ✅ DONE | Acción `login` (correo + `CRUMBLY_LOGIN_PASSWORD`), también resuelta antes del chequeo de token — camino alternativo al de Google. Ambas acciones devuelven el mismo `CRUMBLY_TOKEN` real una sola vez; el token real nunca vive en el código público del frontend. |
| `CrumblySync.loginGoogle()` / `.login()` en `js/sync.js` | ✅ DONE | POST `action=loginGoogle`+`idToken` y POST `action=login`+`email`+`password`, ninguna manda `token` (todavía no lo tienen). 6 tests en `tests/sync.test.js` entre ambas. |
| Pantalla de login (`#login-gate`) en `index.html` | ✅ DONE | Card limpia (reutiliza `.card`) con: botón oficial de Google arriba, divisor "o con tu correo", campos correo+contraseña abajo, y **"¿Problemas para entrar?"** como último recurso (URL+token técnicos) solo si ninguno de los dos logins normales funciona todavía. `handleGoogleCredential()` y `doLogin()` comparten el mismo manejo de error (`#login-error`) y terminan en `connectWithToken()`. |
| `connectWithToken(token, successMsg)` | ✅ DONE | Extraído como helper compartido (guardar config + `pull` + refrescar UI + toast) — lo usan el login con Google, el login con contraseña, el login avanzado y el link mágico (`applySetupLinkIfPresent`), evitando cuadriplicar la misma lógica (`HANDOFF.md` §20). |
...
| Comprobantes reales a Google Drive | ✅ DONE | Acción `uploadComprobante` en el backend (previa a este bloque de login) sube el archivo a una carpeta privada de Drive del dueño del script y devuelve la URL real, guardada en `venta.comprobante`/`gasto.comprobante` en vez de solo el nombre del archivo. `CrumblySync.uploadFile()` con 4 tests. Bug real encontrado al probar el flujo completo: `registrarGasto` en `js/core.js` no incluía `comprobante` en el objeto devuelto (a diferencia de `applyVenta`, que sí) — corregido, con test de regresión. |

Nota de seguridad: el backend nunca confía en el JWT de Google solo por decodificarlo — lo re-verifica contra el propio Google (`tokeninfo`) en cada login, así que un JWT falsificado o de otra app (`aud` distinto) no sirve. La contraseña corta, por su parte, es deliberadamente más débil que el token real — si alguien la adivina, solo puede iniciar sesión, no saltarse el paso y usar el token directamente. Solo la cuenta/correo en `CRUMBLY_LOGIN_EMAIL` puede entrar por cualquiera de los dos caminos. El diseño offline-first (todo se cachea en `localStorage` tras el login) se mantiene a propósito: es lo que permite que la app siga funcionando en el local sin señal, no es un descuido de seguridad.

Verificado en navegador (server estático local, `localStorage` limpio): la card se ve limpia, sin campos técnicos visibles por defecto; el escape hatch URL/token se despliega correctamente al tocar "¿Problemas para entrar?"; el login con correo+contraseña manda la petición correcta y muestra el error del backend cuando corresponde.

**Pendiente del lado del usuario** (documentado en `backend/SETUP.md`): crear un Client ID de OAuth en Google Cloud Console, agregar `CRUMBLY_GOOGLE_CLIENT_ID`/`CRUMBLY_LOGIN_EMAIL`/`CRUMBLY_LOGIN_PASSWORD` a las Propiedades del script, reemplazar el `GOOGLE_CLIENT_ID` placeholder en `index.html` por el real, y desplegar una nueva versión de `backend/Code.gs` (la versión ya desplegada, "Versión 2", solo tiene el login de contraseña sin correo ni Google — obsoleta).

### Autenticación real, sin token fijo (29 de agosto de 2026)

Pedido explícito y con instrucciones técnicas propias del usuario, en dos rondas el mismo día. La primera ronda todavía dejaba un camino opcional de correo+contraseña y auto-registraba usuarios nuevos al primer login con Google. La **segunda ronda, más estricta, es la que quedó vigente**: herramienta de uso interno, sin registro público, sin crear cuenta, sin recuperar contraseña, sin invitaciones automáticas — Google es la ÚNICA puerta, y solo entra un correo que ya esté autorizado a mano en la hoja `usuarios`. Reemplaza por completo el modelo de token fijo (`CRUMBLY_TOKEN` en Script Properties) por **sesiones**: cada login exitoso crea una fila nueva en una hoja `sesiones`, con un token aleatorio propio de ESE dispositivo.

| Pieza | Estado | Verificación |
|---|---|---|
| `js/auth.js` (nuevo archivo) | ✅ DONE | Módulo separado de `js/core.js` y `js/sync.js`, tal como pidió el usuario. Expone `window.auth` con `loginGoogle()`, `logout()`, `getCurrentUser()`, `isAuthenticated()`, `restoreSession()`, `getToken()`, `getBackendUrl()` — sin `loginEmail`, retirado en la segunda ronda. Guarda la sesión en su PROPIA llave de `localStorage` (`crumbly-session`), separada de `crumbly-state` — a propósito: el token de sesión es un dato de este dispositivo, no un dato del negocio, y no debe viajar dentro de un push/pull ni terminar guardado en la hoja de cálculo. 10 tests en `tests/auth.test.js`, incluyendo persistencia entre "recargas" (`restoreSession()` en una instancia nueva del módulo leyendo lo que guardó otra). |
| `authGoogle_(body)` en `backend/Code.gs` | ✅ DONE | Verifica el ID token contra Google (`oauth2.googleapis.com/tokeninfo`: `aud`, `email_verified`), busca el correo en la hoja `usuarios` — si no existe: `{success:false, authorized:false, message:'Usuario no autorizado'}`; si existe con `activo≠TRUE`: `'Usuario desactivado'`; si existe y activo, actualiza `lastLogin` y devuelve `{success:true, authorized:true, user:{id,email,nombre,rol}, token}` — la forma exacta que pidió el usuario, con `token` agregado (imprescindible para que push/pull puedan probar la sesión en cada pedido; no estaba en el JSON de ejemplo). **Nunca crea usuarios** — a diferencia de la primera ronda. |
| `createUser`/`disableUser`/`enableUser` en `backend/Code.gs` | ✅ DONE | Funciones internas, sin acción HTTP ni panel visual (pedido explícito: "No implementar todavía un módulo visual"), pensadas para correrse a mano desde el editor de Apps Script o cablearse a un futuro panel sin reescribirlas. `createUser` asigna un `id` correlativo y `rol:'admin'` por defecto (todos admin por ahora, con `activo`/`rol` preparados para roles operativos futuros, tal como se pidió). |
| Hoja `usuarios` — esquema exacto pedido | ✅ DONE | Columnas `id, email, nombre, rol, activo, createdAt, lastLogin` — sin `foto` (no estaba en el esquema pedido; se sacó de `user` en la respuesta también). Autorizar/desautorizar a alguien es editar esta hoja a mano, o correr las funciones de arriba — nunca algo que dispare la app. |
| `isValidSession_`/`mintSession_`/`logoutSession_` en `backend/Code.gs` | ✅ DONE | Hoja `sesiones` (token, email, creado, expira — 90 días) creada sola la primera vez. `push`/`pull`/`uploadComprobante`/`ping` validan contra esta hoja en cada pedido, ya no contra `CRUMBLY_TOKEN` (eliminado del archivo). `logout` borra la fila de esa sesión — un dispositivo desconectado no puede reusar el token viejo. |
| Pantalla de login rediseñada en `index.html` | ✅ DONE | Logo, "Bienvenido", un único botón de Google — sin "Continuar con correo" (retirado en la segunda ronda, ya no hay contraseña propia de la app que ofrecer). **Se eliminó por completo**: campo de URL, campo de Token, botón "Conectar con token", panel de configuración técnica, y el link mágico `?token=...`. |
| Ajustes → Sincronización | ✅ DONE | Sin campos de URL/Token editables — muestra "Conectado como correo · rol" y conserva "Probar conexión"/"Sincronizar ahora"/"Cerrar sesión", todos operando sobre la sesión de `auth.js`. |
| `state.config.backendUrl`/`backendToken` | ✅ Dejaron de usarse | Los campos siguen existiendo en el esquema de `js/core.js` (no se tocó — migración no destructiva) pero ya no los escribe ni los lee ningún código nuevo. Antes de este cambio viajaban dentro de `state` en cada push/pull — un detalle no evidente hasta ahora: el token de sesión de un dispositivo terminaba guardado en la hoja `state_json`. Con la sesión movida a `auth.js`/`crumbly-session`, ya no ocurre. |

**"No usar listas de emails dentro del frontend" (pedido explícito):** la autorización nunca vivió en el frontend — vivió primero en una Script Property (`CRUMBLY_LOGIN_EMAIL`, comma-separated) en la primera ronda, y en la segunda ronda se movió a la hoja `usuarios`, que es más apegado al pedido ("toda la autorización debe vivir en Google Sheets y Apps Script") porque no requiere editar Script Properties para dar de alta a alguien — es una fila de spreadsheet, con nombre, rol y auditoría de último acceso.

**Diseño de multi-dispositivo:** con un solo `CRUMBLY_TOKEN` compartido (el modelo anterior a esta ronda), no existía el concepto de "dispositivo" — todos usaban el mismo secreto. Con `sesiones` como hoja aparte (una fila por login), cada dispositivo tiene su propio token y su propia fecha de expiración; cerrar sesión en el celular no desconecta la caja registradora.
...
**✅ Desplegado (29 ago 2026):** el usuario pidió explícitamente hacerlo en el navegador ("hazlo tu, en el navegador") y confirmó su propia sesión de Google abierta primero — a partir de ahí: proyecto de Cloud Console creado, Client ID generado, `CRUMBLY_GOOGLE_CLIENT_ID` en Script Properties, `backend/Code.gs` como Versión 3, `GOOGLE_CLIENT_ID` real en `index.html`, y `crumbly2026@gmail.com` cargado como primer usuario (`admin`) vía `createUser()`.

**Dos bugs reales encontrados y corregidos al verificar en el navegador real del usuario** (no en el sandbox — ver detalle completo en `HANDOFF.md` § "29 de agosto de 2026"):
1. El botón de Google nunca se dibujaba — `initGoogleSignIn()` solo se intentaba una vez, antes de que el script de Google terminara de cargar (async), sin reintento. Fix: el `<script>` del GSI llama a `initGoogleSignIn()` en su propio `onload`.
2. El login fallaba con `"no se pudo verificar el token de Google"` — el backend no tenía el permiso de red (`script.external_request`) que `authGoogle_()` necesita para consultar `oauth2.googleapis.com/tokeninfo`; la autorización previa solo cubría Sheets. Fix: se revocó y volvió a autorizar el script con los 3 scopes juntos (Sheets + Drive + red externa).

Verificado con `fetch` real desde el navegador contra el backend en producción: un `idToken` falso ahora devuelve `"token de Google inválido"` (validación real contra Google), no el error de red de antes. Pendiente: que el usuario complete un login real tocando el botón — lo único que ningún automatismo de este entorno puede hacer por él.

### Pulido del login real (30 de agosto de 2026)

El usuario confirmó que el login con Google **ya funciona de punta a punta** en su navegador real, y reportó dos fricciones de uso:

| Pieza | Estado | Verificación |
|---|---|---|
| Indicador "Conectando…" | ✅ DONE | Tras tocar el botón de Google no había ninguna señal de que algo estaba pasando — parecía trabado. Se agrega un texto "Conectando…" bajo el botón mientras el backend valida la sesión (`login-status`, mostrado/ocultado en `handleGoogleCredential`). |
| El ícono de usuario no hacía nada | ✅ DONE | Ahora abre Ajustes (mismo modal que el engranaje) — ahí vive "Cerrar sesión en este dispositivo". Antes era puramente decorativo. |
| "Sincronización" en Ajustes | ✅ Simplificado | A pedido explícito ("la sincronización debe ser automática"): se quitan los botones "Probar conexión"/"Sincronizar ahora" — la sincronización ya es automática (push tras cada guardado, pull al iniciar sesión) y los botones manuales quedaban redundantes/confusos. Queda solo el estado ("Conectado como… · rol", "Sincronizado ✓ · hora") como feedback pasivo. |
| "Zona de riesgo" (resetear datos de prueba) | ✅ Quitado del UI | Herramienta de desarrollo, no algo para la app ya lista para lanzar — a pedido explícito. |
| "Importar menú Crumbly 2026" | ✅ Quitado del UI | Mismo motivo — era una herramienta de una sola vez para la carga inicial del menú. La función `importMenuCrumbly()` queda en el código (sin caller, comentario `ponytail:`) por si hace falta reimportar alguna vez, callable desde la consola del navegador — mismo patrón que `createUser()`/`disableUser()`/`enableUser()` en `backend/Code.gs`. |

Con esto, Ajustes quedó reducido a lo que realmente hace falta: reportes descargables, estado de sincronización + cerrar sesión, correo de la marca, y nada más.

### Bugs reales encontrados importando datos de verdad (7 de septiembre de 2026)

El usuario empezó a cargar su inventario y menú reales y fue reportando, con capturas, cada cosa que no cuadraba. Ronda larga, siete arreglos — quedan documentados acá en el orden en que se encontraron:

| # | Problema reportado | Causa real | Fix |
|---|---|---|---|
| 1 | Márgenes absurdos (`-1.354.057%`, luego `-2.503.896%` en TODOS los productos) | No era un bug de fórmula — `getCostoProducto`/`getMargenProducto` siempre estuvieron bien. La causa real: el costo por unidad de un insumo se cargaba con el precio del **paquete completo** en vez de por gramo/unidad individual (confirmado por el usuario: "el precio de la harina es de 10500, del total, no 10500 por gramo"). | Ver #6 y #7 — la causa raíz se resuelve rediseñando cómo se carga el costo, no parcheando el cálculo. |
| 2 | "No se puede subir la foto" + "No se pudo guardar — almacenamiento lleno" | Las fotos de producto se guardaban como **base64 completo dentro de `state`** → `localStorage` — una sola foto de celular llenaba la cuota y `saveState()` empezaba a fallar para todo lo demás. Además, aun subida, `file.getUrl()` de Drive da la página visor (HTML), no sirve como `src` de `<img>`. | Las fotos ahora se suben a Drive vía el backend (mismo mecanismo que los comprobantes) y se comparten "cualquiera con el link puede ver" — `producto.foto` guarda una URL de contenido directo (`lh3.googleusercontent.com`), no la imagen completa. Ver `backend/Code.gs` § `uploadComprobante_`. |
| 3 | "Huevos" (materia prima) apareciendo en Empaques, y varios más (Belga preparados, Croissants, Banano) | `saveInsumo()` inferá la categoría del insumo a partir de la **unidad de medida** ("unidad" ⇒ empaque, cualquier otra cosa ⇒ materia) — cualquier ingrediente contado por unidad caía mal. | Selector explícito "Tipo de insumo" (Materia prima/Empaque), independiente de "Tipo de medición". Los ya mal ubicados hay que moverlos a mano (borrar y recrear) — el fix es solo hacia adelante. |
| 4 | Scroll horizontal en las filas de "Fórmula" del producto | `display:flex` sin `flex-wrap` — dos `<select>` de texto largo no cabían en una pantalla angosta. | `flex-wrap:wrap` + `min-width` razonable en vez de `min-width:0` (que dejaba encoger los `<select>` hasta ilegibles en vez de pasar a una segunda línea — bug del primer intento de este mismo fix, corregido en el commit siguiente). |
| 5 | Filtros de Inventario confusos ("¿así los estás organizando?") | Los chips eran solo "categoría" (texto libre por insumo) — un chip llamado "Empaque" era una categoría más entre muchas, no un filtro real por tipo. | Nueva fila de filtro por Tipo real (Materia prima/Empaque/Toppings, según en qué colección vive cada insumo), con la fila de categoría acotada al tipo elegido. |
| 6 | Decimales en precios, márgenes y cantidades ("no dejes decimales") | Varios `toFixed(1)`/`toFixed(2)` sin redondear, y `precio`/`monto` no se forzaban a entero al guardar. | `Math.round()` en el margen del modal de producto, en el selector de insumo de "Registrar gasto" (ahora usa `fmt()`), y al guardar `precio`/`monto`. También en 6 lugares donde se lista una `cantidad` (Inventario, alertas, PDFs de mermas) — solo el *display*, no el valor interno (las preparaciones necesitan precisión fraccionaria real). |
| 7 | Causa raíz del margen absurdo, confirmada por el usuario | "Harina fuerza" a \$10.500/g — el precio del paquete completo cargado directo como costo por gramo. | Dos pasos: (a) aviso de "costo sospechoso" en Inventario con umbral distinto por unidad (\$200/g o /ml, \$50.000/kg o /unidad — un umbral único no marcaba esto); (b) **el fix real**, a pedido explícito ("la calculadora debe automáticamente definir el valor x unidad"): "Precio del paquete" + "Contenido del paquete" pasan a ser los primeros campos del formulario de insumo, no un cálculo opcional escondido — "Costo por unidad" queda como resultado calculado, ajustable a mano si hace falta. |

Verificado en cada paso con `node --check`, la suite completa de tests (140/140: 115 core + 15 sync + 10 auth — 3 tests nuevos para `metodoPago`), y prueba manual en navegador (servidor local, sesión simulada) antes de cada push. Además, nuevo: **"Medio de pago"** en Registrar gasto (Efectivo/Transferencia/Dividido, con dos montos que deberían sumar el total pagado — aviso, no bloqueo, si no cuadran), mismo campo que ya usa `registrarVenta` (`metodoPago`), extendido con `'dividido'`.

**Pendiente del lado del usuario:** corregir a mano, insumo por insumo, los que ya quedaron mal cargados antes de estos fixes (usando la calculadora nueva y el selector de Tipo de insumo) — el código ya no permite que vuelva a pasar, pero no migra datos existentes.
