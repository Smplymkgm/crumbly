...
### Login (25 de agosto de 2026) — de correo+contraseña a "Iniciar sesión con Google"

El usuario insistió en ir más allá del link de un toque: quería que cualquier dispositivo nuevo solo necesitara "ingresar y todo esté listo, nada local" — sin copiar URL/token a mano ni depender de un link guardado. Primero se construyó un login con correo + contraseña corta (decisión explícita del usuario de no reutilizar el token largo como contraseña). El mismo día, antes de que esa versión llegara a desplegarse, el usuario pidió ir un paso más: iniciar sesión con su cuenta de Google en vez de inventar una contraseña — se reemplazó el formulario por **Google Identity Services**, con el trade-off (requiere un proyecto en Google Cloud Console) explicado y confirmado por el usuario antes de construirlo.

| Pieza | Estado | Verificación |
|---|---|---|
| `loginGoogle_(body)` en `backend/Code.gs` | ✅ DONE | Acción `loginGoogle`, resuelta en `doPost` **antes** del chequeo de token (es la única acción que no lo requiere — es la que lo entrega). Recibe el ID token (JWT) que Google entrega en el navegador y lo **verifica contra Google** (`oauth2.googleapis.com/tokeninfo`, no confía en decodificarlo solo) — chequea `aud` contra `CRUMBLY_GOOGLE_CLIENT_ID`, `email_verified`, y que el correo sea exactamente `CRUMBLY_LOGIN_EMAIL`. Si todo coincide, devuelve el `CRUMBLY_TOKEN` real una vez. El token real nunca vive en el código público del frontend. |
| `CrumblySync.loginGoogle()` en `js/sync.js` | ✅ DONE | POST `action=loginGoogle` con `idToken`, sin `token` (todavía no lo tiene). 3 tests en `tests/sync.test.js` (mandar el campo correcto, devolver el token, propagar `ok:false` si el correo no es el autorizado). |
| Pantalla de login (`#login-gate`) en `index.html` | ✅ DONE | Pantalla completa que tapa la app hasta que hay sesión. Botón oficial de Google (`google.accounts.id.renderButton`), `handleGoogleCredential()` recibe el JWT y llama a `CrumblySync.loginGoogle`; si el backend lo confirma, usa el token devuelto para conectar (`connectWithToken`) y traer los datos. Enlace **"¿Problemas para entrar?"** abre los campos técnicos (URL+token) como alternativa (`doLoginAvanzado`), para no dejar a nadie bloqueado si el backend real todavía no tiene Google Sign-In configurado. |
| `connectWithToken(token, successMsg)` | ✅ DONE | Extraído como helper compartido (guardar config + `pull` + refrescar UI + toast) — lo usan el login con Google, el login avanzado y el link mágico (`applySetupLinkIfPresent`), evitando triplicar la misma lógica (`HANDOFF.md` §20). |
...
Nota de seguridad: el backend nunca confía en el JWT de Google solo por decodificarlo — lo re-verifica contra el propio Google (`tokeninfo`) en cada login, así que un JWT falsificado o de otra app (`aud` distinto) no sirve. Solo la cuenta de `CRUMBLY_LOGIN_EMAIL` puede entrar. El diseño offline-first (todo se cachea en `localStorage` tras el login) se mantiene a propósito: es lo que permite que la app siga funcionando en el local sin señal, no es un descuido de seguridad.

**Pendiente del lado del usuario** (documentado en `backend/SETUP.md`): crear un Client ID de OAuth en Google Cloud Console, agregar `CRUMBLY_GOOGLE_CLIENT_ID`/`CRUMBLY_LOGIN_EMAIL` a las Propiedades del script, reemplazar el `GOOGLE_CLIENT_ID` placeholder en `index.html` por el real, y desplegar una nueva versión de `backend/Code.gs` (la versión ya desplegada, "Versión 2", es la de correo+contraseña — ya obsoleta, nunca llegó a usarse contra un backend real).
