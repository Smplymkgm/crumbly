...
Pedido explícito y con instrucciones técnicas propias del usuario (contexto técnico + objetivo + qué eliminar + forma del flujo + `window.auth` como módulo separado), a continuación de un intercambio en el que preguntó si el login podía ser "con Google" en vez de correo+contraseña. Reemplaza por completo el modelo de token fijo (`CRUMBLY_TOKEN` en Script Properties, copiado a mano o intercambiado por login) por **sesiones**: cada login exitoso (Google o, opcional, correo+contraseña) crea una fila nueva en una hoja `sesiones`, con un token aleatorio propio de ESE dispositivo — cerrar sesión en uno no afecta a los demás, y no hay ningún secreto fijo que un dispositivo comprometido pueda usar para siempre.

| Pieza | Estado | Verificación |
|---|---|---|
| `js/auth.js` (nuevo archivo) | ✅ DONE | Módulo separado de `js/core.js` y `js/sync.js`, tal como pidió el usuario. Expone `window.auth` con `loginGoogle()`, `loginEmail()` (opcional), `logout()`, `getCurrentUser()`, `isAuthenticated()`, `restoreSession()`, `getToken()`, `getBackendUrl()`. Guarda la sesión en su PROPIA llave de `localStorage` (`crumbly-session`), separada de `crumbly-state` — a propósito: el token de sesión es un dato de este dispositivo, no un dato del negocio, y no debe viajar dentro de un push/pull ni terminar guardado en la hoja de cálculo. 11 tests en `tests/auth.test.js`, incluyendo persistencia entre "recargas" (`restoreSession()` en una instancia nueva del módulo leyendo lo que guardó otra). |
| `authGoogle_(body)` en `backend/Code.gs` | ✅ DONE | Verifica el ID token contra Google (`oauth2.googleapis.com/tokeninfo`: `aud`, `email_verified`), chequea el correo contra `CRUMBLY_LOGIN_EMAIL` (uno o más separados por coma, revisado en CADA login, no solo al crear la cuenta), busca/crea al usuario en la hoja `usuarios`, y devuelve `{ success, user:{id,email,nombre,foto,rol}, token }` — la forma exacta que pidió el usuario, con `token` agregado (imprescindible para que push/pull puedan probar la sesión en cada pedido; no estaba en el JSON de ejemplo del pedido). |
| `login_(body)` en `backend/Code.gs` (opcional) | ✅ DONE | Camino de correo+contraseña — pedido explícitamente como "Opcional" en la pantalla de login. Mismo mecanismo de sesión que Google (`mintSession_`), mismo chequeo de `CRUMBLY_LOGIN_EMAIL`. `CRUMBLY_LOGIN_PASSWORD` sigue siendo un secreto propio de la app, sin relación con contraseñas de Google. |
| `isValidSession_`/`mintSession_`/`logoutSession_` en `backend/Code.gs` | ✅ DONE | Hoja `sesiones` (token, email, creado, expira — 90 días) creada sola la primera vez. `push`/`pull`/`uploadComprobante`/`ping` validan contra esta hoja en cada pedido, ya no contra `CRUMBLY_TOKEN` (que se eliminó del archivo — ni la variable ni la Script Property se usan más). `logout` (acción nueva) borra la fila de esa sesión — un dispositivo desconectado no puede reusar el token viejo. |
| Rol automático (`dueño`/`staff`) | ✅ DONE | El primer usuario que se registra en la hoja `usuarios` queda como `dueño`; los siguientes, `staff`. Sin pantalla de administración — el rol es una celda más, editable a mano directamente en la hoja, igual que el resto de los datos de este backend. |
| Pantalla de login rediseñada en `index.html` | ✅ DONE | Logo, "Bienvenido", botón de Google, y "Continuar con correo" opcional/colapsado — tal como se pidió, sin ningún campo técnico visible. **Se eliminó por completo**: el campo de URL del backend, el campo de Token, el botón "Conectar con token", la pantalla/panel de configuración técnica ("¿Problemas para entrar?"), y el link mágico `?token=...` (dejó de tener sentido: no hay ningún secreto fijo que compartir por URL en el modelo de sesiones). |
| Ajustes → Sincronización | ✅ DONE | Ya no tiene campos de URL/Token editables — muestra "Conectado como correo · rol" y conserva "Probar conexión"/"Sincronizar ahora"/"Cerrar sesión", todos operando sobre la sesión de `auth.js` en vez de sobre `state.config.backendUrl/backendToken`. |
| `state.config.backendUrl`/`backendToken` | ✅ Dejaron de usarse | Los campos siguen existiendo en el esquema de `js/core.js` (no se tocó — migración no destructiva) pero ya no los escribe ni los lee ningún código nuevo; quedan vacíos para siempre en instalaciones nuevas. Antes de este cambio, viajaban dentro de `state` en cada push/pull — un detalle no evidente hasta ahora: el token de sesión de un dispositivo terminaba guardado en la hoja `state_json`. Con la sesión movida a `auth.js`/`crumbly-session`, ya no ocurre. |

**Diseño de multi-dispositivo, explícito:** con un solo `CRUMBLY_TOKEN` compartido (el modelo anterior), no existía el concepto de "dispositivo" — todos usaban el mismo secreto. Con `sesiones` como hoja aparte (una fila por login, no una celda por usuario), cada dispositivo tiene su propio token y su propia fecha de expiración; cerrar sesión en el celular no desconecta la caja registradora.

Verificado: 138/138 tests automatizados (`core.test.js` 112 + `sync.test.js` 15 + `auth.test.js` 11) y flujo manual completo en navegador (servidor local, `fetch` simulado): pantalla de login limpia sin campos técnicos, "Continuar con correo" → login → dashboard con el nombre real en el saludo y en Ajustes, sesión sobrevive a recargar la página (`auth.restoreSession()`), "Cerrar sesión" borra la sesión local y vuelve a la pantalla de login, y el botón de Google se dibuja correctamente una vez que el script de Google Identity Services termina de cargar.

**Pendiente del lado del usuario** (documentado en `backend/SETUP.md`): redesplegar `backend/Code.gs` como nueva versión (reemplaza por completo el modelo de token — es la versión más importante de todo este proyecto para desplegar), y si no se hizo antes, crear el Client ID de OAuth en Google Cloud Console para que el botón de Google funcione contra el backend real. El correo+contraseña opcional ya funciona con solo `CRUMBLY_LOGIN_EMAIL`/`CRUMBLY_LOGIN_PASSWORD` configurados, sin pasar por Cloud Console.
...
**`auth.test.js`: 11/11 pasan.** Autenticación (Google, correo+contraseña opcional, sesión, logout) con `fetch` y `localStorage` simulados — ver tabla de "Autenticación real, sin token fijo" arriba.
