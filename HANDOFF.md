...
El usuario pidió, en dos rondas el mismo día, reemplazar por completo el modelo de token del punto 5 anterior. La primera ronda (instrucciones técnicas propias: contexto del stack, qué eliminar, forma de la pantalla, flujo, `window.auth` como módulo separado) todavía dejaba correo+contraseña como camino opcional y auto-registraba usuarios nuevos al primer login con Google. La **segunda ronda, más estricta y la que quedó vigente**, llegó con un encabezado explícito: "La aplicación Crumbly es una herramienta de uso interno" — y una lista concreta de lo que NO debe existir: registro público, crear cuenta, recuperar contraseña, invitaciones automáticas. Ambas rondas fueron explícitas en no introducir React/Vite/Webpack/TypeScript ni ningún framework — la app sigue siendo HTML+JS puro, sin build step, servida tal cual en GitHub Pages.

**Qué quedó, en una frase:** Google es la ÚNICA puerta, y solo entra un correo que ya esté autorizado A MANO en una hoja `usuarios` — el login nunca crea usuarios, nunca ofrece un camino alternativo, y no hay ningún secreto fijo (`CRUMBLY_TOKEN`) que un dispositivo obtenga una vez y use para siempre. Cada login válido crea una **sesión** nueva — una fila en una hoja `sesiones` del backend, con su propio token aleatorio y su propia fecha de expiración (90 días) — y push/pull/uploadComprobante validan esa fila en cada pedido. Cerrar sesión en un dispositivo (o borrar su fila a mano en la hoja) no afecta a los demás.

Piezas: `js/auth.js` (módulo de autenticación, `window.auth`, con sus propios 10 tests, sin `loginEmail`), la hoja `usuarios` (esquema exacto pedido: `id, email, nombre, rol, activo, createdAt, lastLogin` — sin `foto`), `createUser`/`disableUser`/`enableUser` como funciones internas de Apps Script (sin panel visual, a propósito — pedido explícito, pensadas para cablearse a un panel el día que exista), la hoja `sesiones`, y una pantalla de login con un único botón de Google, sin nada más — se eliminaron el campo de URL, el campo de Token, el botón "Conectar con token", el panel de configuración avanzada, el link mágico `?token=...`, y (en la segunda ronda) también el formulario de correo+contraseña. Detalle pieza por pieza, con la tabla completa de verificación, en `IMPLEMENTATION_STATUS.md` § "Autenticación real, sin token fijo".

**"Toda la autorización debe vivir en Google Sheets y Apps Script" (pedido explícito):** por eso quién puede entrar no es una Script Property (una lista de correos separados por coma, como en la primera ronda) sino una hoja de cálculo — autorizar o desautorizar a alguien es editar una fila, con nombre, rol y auditoría de último acceso, no tocar configuración técnica.
...
Verificado: 137/137 tests automatizados (112 core + 15 sync + 10 auth) y flujo manual completo en navegador: pantalla de login con un solo botón, mensaje claro cuando el backend rechaza un correo no autorizado, login exitoso de punta a punta con `fetch` simulado, sesión sobreviviendo a recargar la página, y el botón de Google renderizando correctamente.

**✅ Desplegado el 29 de agosto de 2026, con permiso explícito del usuario** ("hazlo tu, en el navegador" — sesión de Google ya abierta por el propio usuario, jamás por mí): proyecto "Crumbly" creado en Google Cloud Console, pantalla de consentimiento OAuth y Client ID configurados, `CRUMBLY_GOOGLE_CLIENT_ID` agregado a Script Properties (y `CRUMBLY_TOKEN`, ya obsoleto, eliminado), `backend/Code.gs` redesplegado como Versión 3, `GOOGLE_CLIENT_ID` real pegado en `index.html`, y el primer usuario (`crumbly2026@gmail.com`, rol `admin`) cargado en la hoja `usuarios` corriendo `createUser()` desde el editor.

**Bug real #1 (reportado por el usuario con captura): el botón de Google nunca aparecía.** `initGoogleSignIn()` solo se llamaba una vez, desde `showLoginGate()` al arrancar — pero el script de Google (`gsi/client`) carga async y casi nunca está listo tan rápido, así que ese primer intento fallaba en silencio (`typeof google === 'undefined'`) y no había ningún reintento. El botón quedaba vacío para siempre en cualquier carga normal — reproducido tanto en Chrome como en Safari del usuario, incluso en incógnito y con caché borrada, confirmando que no era un problema de caché sino del código. **Fix de una línea:** el `<script>` del GSI ahora llama a `initGoogleSignIn()` en su propio evento `onload` — ese es el intento que realmente dibuja el botón. Verificado en local antes y después del fix (sin fix: vacío incluso 2.5s después de cargar; con fix: aparece solo).
