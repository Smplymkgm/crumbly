...
**Pendiente del lado del usuario:** redesplegar `backend/Code.gs` como nueva versión, y agregar cada correo autorizado en DOS lugares distintos — la hoja `usuarios` (autorización propia de la app) y "Usuarios de prueba" en la pantalla de consentimiento de OAuth de Google Cloud Console (requisito de Google, independiente de la app). Runbook actualizado en `backend/SETUP.md`.
